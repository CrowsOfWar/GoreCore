...
public final class GoreCoreClientEvents {
	...
	/**
	 * Gets the occurence of symbol sign that is the last one
	 * before a normal character. If the symbol is not present,
	 * etc, this returns 0.
	 */
	...
	public void onClientChat(ClientChatReceivedEvent event) {
		if (event.message.getUnformattedText().startsWith("[gc-format]")) {
			...
			try {
				...
				FMLLog.warning("GoreCore> Caught a formatting error while using multicommand. " +
						"Check your lang file if you're localizing!");
				e.printStackTrace();
			}
			
			// Make sure that each word has a \u00a7{code} before it to prevent losing colors on new line
			String[] split = nooText.split(" "); // nooText split between spaces
			String[] splitResult = new String[split.length]; // split but every word has \u00a7{code} before it
			...
			nooText = "";
			for (String str : splitResult) nooText += str + " ";
		...
		// To get GoreCore formatting, apply [gc-format] to my.chat.1, but not my.chat.2 or my.chat.3
		...
		if (text.startsWith("[gc-combine]")) {
			...
			if (keysToSend.length >= 1) {
				...
				for (int i = 0; i < params.length; i++) params[i] = GoreCore.proxy.translate(keysToSend[i + 1],
						translateParams);
